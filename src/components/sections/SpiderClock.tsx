...
export default function SpiderClock() {
  ...
  useEffect(() => {
    ...
    // Load GSAP Core first, then MorphSVGPlugin
    loadScript('https://cdnjs.cloudflare.com/ajax/libs/gsap/3.12.2/gsap.min.js')
      .then(() => loadScript('https://unpkg.com/gsap@3/dist/MorphSVGPlugin.min.js'))
      .then(() => {
        if (!active) return;
        ...
        if (!gsap || !MorphSVGPlugin) {
          throw new Error('GSAP or MorphSVGPlugin failed to load');
        }
        ...
        // Start animations after short delay to ensure rendering is complete
        setTimeout(() => {
          if (!active || !containerRef.current) return;
          initializeAnimation(gsap);
        }, 100);
      })
      .catch((err) => {
        console.error('SpiderClock script loading failed:', err);
        if (active) {
          setError(true);
          setLoading(false);
        }
    ...
    const initializeAnimation = (gsap: any) => {
      const select = (e: string) => containerRef.current?.querySelector(e) as any;
      ...
      const face02 = select("#face02")?.getAttribute("d") || "";

      const handSec01 = select("#handSec01")?.getAttribute("d") || "";
      const handSec02 = select("#handSec02")?.getAttribute("d") || "";
      const sec = select("#sec");

      const handMin01 = select("#handMin01")?.getAttribute("d") || "";
      const handMin02 = select("#handMin02")?.getAttribute("d") || "";
      const min = select("#min");

      const handHr01 = select("#handHr01")?.getAttribute("d") || "";
      const handHr02 = select("#handHr02")?.getAttribute("d") || "";
      const hr = select("#hr");
      ...
      function geSecRotation() {
        ...
        const difference = Math.abs(gsap.getProperty(sec, "rotation") - rotation);
      ...
      function getMinRotation() {
        ...
        const difference = Math.abs(gsap.getProperty(min, "rotation") - rotation);
      ...
      function getHrRotation() {
        ...
        const difference = Math.abs(gsap.getProperty(hr, "rotation") - rotation);
      ...
      gsap.to(containerRef.current?.querySelectorAll(".gsapWrapper, .vline"), {
        autoAlpha: 1,
        duration: 0.5
      });

      // Cog rotations
      const t1 = gsap.to(containerRef.current?.querySelectorAll(".cw.t24"), {
        rotation: "-=15",
      ...
      const t2 = gsap.to(containerRef.current?.querySelectorAll(".cw.t20"), {
        rotation: "-=18",
        transformOrigin: "50% 50%",
        ease: "bounce.out",
        duration: 1,
      ...
      const t3 = gsap.to(containerRef.current?.querySelectorAll(".ccw.t12"), {
        rotation: "+=30",
        transformOrigin: "50% 50%",
        ease: "bounce.out",
        duration: 1,
      ...
      // Minute hand ticking tween
      const tMin = gsap.to(min, {
      ...
      // Hour hand ticking tween
      const tHr = gsap.to(hr, {
      ...
      // Second hand ticking tween
      const tSec = gsap.to(sec, {
        ...
        onComplete: function () {
          ...
          this.invalidate().delay(0.5).restart(true);
        }
      });
      timelines.push(tSec);

      // Breathing face morph timeline
      ...
      // Twitching second leg morph timeline
      const tg1 = gsap.timeline({ repeat: -1, repeatDelay: 5, defaults: { duration: 1.5, ease: "bounce.out" } })
        .delay(1)
        .call(() => {
          const rotation = parseFloat(gsap.getProperty(sec, "rotation").toFixed(1));
          if ((rotation > 30 && rotation < 150) || (rotation > 210 && rotation < 330)) {
            gsap.timeline({ repeat: 0, defaults: { duration: 0.25, ease: "bounce.in" } })
              .to(handSecEl, { morphSVG: handSec02 })
              .to(handSecEl, { morphSVG: handSec01 });
          }
        })
        .set(sec, {
          onComplete() {
      ...
      // Twitching minute leg morph timeline
      const tg2 = gsap.timeline({ repeat: -1, repeatDelay: 5, defaults: { duration: 1.5, ease: "bounce.out" } })
        .delay(5)
        .call(() => {
          const rotation = parseFloat(gsap.getProperty(min, "rotation").toFixed(1));
          if ((rotation > 5 && rotation < 175) || (rotation > 185 && rotation < 355)) {
            gsap.timeline({ repeat: 0, defaults: { duration: 0.25, ease: "bounce.in" } })
              .to(handMinEl, { morphSVG: handMin02 })
              .to(handMinEl, { morphSVG: handMin01 });
          }
        })
        .set(min, {
          onComplete() {
      ...
      // Twitching hour leg morph timeline
      const tg3 = gsap.timeline({ repeat: -1, repeatDelay: 5, defaults: { duration: 1.5, ease: "bounce.out" } })
        .delay(7)
        .call(() => {
          const rotation = parseFloat(gsap.getProperty(hr, "rotation").toFixed(1));
          if ((rotation > 2 && rotation < 178) || (rotation > 182 && rotation < 358)) {
            gsap.timeline({ repeat: 0, defaults: { duration: 0.25, ease: "bounce.in" } })
              .to(handHrEl, { morphSVG: handHr02 })
              .to(handHrEl, { morphSVG: handHr01 });
          }
        })
        .set(hr, {
          onComplete() {
  ...
  return (
    <div ref={containerRef} className="spider-clock-container relative flex flex-col justify-center items-center w-full min-h-[400px] sm:min-h-[500px] bg-black/35 backdrop-blur-md rounded-2xl border border-white/[0.04] p-4 overflow-hidden shadow-inner select-none">
      
      {/* Dynamic Background Glows */}
      <div className="absolute top-[10%] left-[10%] w-[120px] h-[120px] rounded-full bg-accent-purple/[0.1] blur-[40px] pointer-events-none" />
      <div className="absolute bottom-[10%] right-[10%] w-[120px] h-[120px] rounded-full bg-accent-cyan/[0.08] blur-[40px] pointer-events-none" />

      {/* Styled Scoped CSS for watch and SVGs */}
      <style>{`
        .vline {
          position: absolute;
          top: 0;
        ...
        /* Ambient styling for SVG numbers */
        #clockFace {
      ...
      {loading && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-black/20 z-10 transition-all duration-300">
          <div className="w-8 h-8 rounded-full border border-accent-purple/20 border-t-accent-purple animate-spin" />
          <span className="text-[11px] font-mono text-text-secondary tracking-widest uppercase animate-pulse">Loading Creative Sandbox...</span>
        </div>
      )}

      {error && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-black/40 z-10 text-center p-6">
          <span className="text-[24px]">⚠️</span>
          <span className="text-[13px] font-mono text-red-400 font-bold">Failed to load animation libraries</span>
          <p className="text-[11px] text-text-muted max-w-[300px]">Please check your network connection or reload the page to initialize the interactive Spider Clock.</p>
        </div>
      )}
        ...
        <div id="wBody">
          <div id="watch">
            <svg id="watchSVG" overflow="visible" version="1.1" viewBox="0 0 300 300" xmlnsXlink="http://www.w3.org/1999/xlink">
              {/* Spinning Gears inside Clock Face */}
              <g transform="matrix(0.97,0.02,-0.02,0.97,19.19,-6.07)">
                <use className="cog cw t24" fillOpacity="0.12" href="#cog1" />
              </g>
              <use className="cog ccw t12" href="#cog2" fillOpacity="0.15" />
              <g transform="matrix(0.93,0,0,0.93,9.460262,5.75)">
                <use className="cog cw t24" fillOpacity="0.12" href="#cog1" transform="rotate(-51.810031,132.27636,-30.515465)" />
              </g>
              <g transform="rotate(-3.7621103,160.78969,349.2265)">
                <use className="cog cw t20" fillOpacity="0.12" href="#cog5" />
              </g>
              <g transform="rotate(-2.7463672,162.45282,88.573956)">
                <use className="cog ccw t12" fillOpacity="0.15" transform="rotate(-117.25759,103.54258,173.50156)" />
              </g>
              <g transform="rotate(-10.176195,143.10035,73.854981)">
                <use className="cog ccw t12" fillOpacity="0.15" transform="rotate(-34.224386,-107.18833,39.101999)" />
              </g>

              {/* Blurred Glow Background Gears */}
              <use className="cog ccw t12" fillOpacity="0.04" href="#cog5" filter="url(#blur1)" transform="matrix(1.756136,-0.11547602,0.11547602,1.756136,-258.10202,-151.7933)" />
              <use className="cog cw t24" fillOpacity="0.03" href="#cog1" filter="url(#blur2)" transform="matrix(2.2837863,0.04708838,-0.04708838,2.2837863,58.766756,-235.77904)" />
              <use className="cog ccw t12" fillOpacity="0.02" href="#cog1" filter="url(#blur3)" transform="matrix(4.1933711,0.08646126,-0.08646126,4.1933711,-272.22866,-548.76437)" />

              {/* Dial Face Numbers */}
              <use id="clockFace" fill="#FFFFFF" fillOpacity="0.4" stroke="none" href="#arabic" filter="url(#shadow)" />

              {/* Spider Element in Center */}
      ...
      {/* SVG Definitions containing Morph Targets, Cog shapes and numbers */}
      <svg width="0" height="0" role="none" style={{ position: 'absolute' }} xmlns="http://www.w3.org/2000/svg">
        <defs>
